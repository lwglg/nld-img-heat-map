...
class EventHandlerValidator:
    ...
    async def validate(self, event: type[BaseEvent], param: BaseModel = None) -> NoReturn | None:
...
class EventHandler:
    def __init__(self, validator: EventHandlerValidator):
        self.events: dict[BaseEvent, BaseModel | None] = {}
...
class EventHandlerMeta(type):
    ...
    def _get_event_handler(self) -> EventHandler | NoReturn:
        try:
